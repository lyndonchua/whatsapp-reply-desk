...
const cats = ['work','family','personal'];
const catTitle = { work:'Work', family:'Family', personal:'Personal' };

const demo = [
  { id:'ali', sender:'Mr Ali', category:'work', priority:'HIGH', messages:[{time:'08:15', text:'Can you confirm the time for DSA trials on 3 July?'}] },
...
function guessCategory(sender, text){
  ...
  if(/mr |ms |mrs |mdm|teacher|school|student|class|hockey|dsa|econs|admin|dept|parent|coach|sajc|26a|26s|25a|25s/.test(v)) return 'work';
  return 'personal';
}
...
function App(){
  ...
  function combineSelected(){ const ids=[...selected]; if(ids.length<2) return; const chosen=chats.filter(c=>selected.has(c.id)); const base={...chosen[0], sender: chosen.map(c=>c.sender).join(' + '), messages: chosen.flatMap(c=>c.messages)}; setChats(cs=>[...cs.filter(c=>!selected.has(c.id)), removeDupes(base)]); setSelected(new Set()); }
  ...
  return <div className="app">
    <aside className="side">
      ...
      <div className="legend"><span className="dot work"/>Work <span className="dot family"/>Family <span className="dot personal"/>Personal</div>
    ...
    <main className="board">
      {cats.map(cat=><section className={`col ${cat}`} key={cat}><h2>{catTitle[cat]}</h2><div className="cards">
